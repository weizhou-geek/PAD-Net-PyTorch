...
def group_crop_pad(img_group):
    ...
    for img in img_group:
        assert (img.shape[0] == h and img.shape[1] == w)
        outs.append(crop(img,[0,0,th*8,tw*8]))
    return outs
...
def rescale(img, scales, interpolation=cv2.INTER_CUBIC, dsize=None):
...
def group_rescale(img_group, scales, interpolations, dsize=None):
    ...
    for img in img_group:
        outs.append(rescale(img, scale, interpolations, dsize))
...
def group_random_fliplr(img_group):
    if random.random() < 0.5:
        return [fliplr(img) for img in img_group]
    else:
        return img_group

def group_random_flipud(img_group):
    if random.random() < 0.5:
        return [flipud(img) for img in img_group]
    else:
        return img_group
